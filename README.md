...
- 物业报修：提交故障类型、描述和照片，物业接单并更新进度；支持工单转派（写明原因，目标人员接受后才切换归属、拒绝则回到原处理人，待接受期间禁止处理或再次转派）。
- 角色区分：房东、租客、物业人员拥有不同工作台。
- 物业登录：物业人员通过账号密码登录（JWT），接单、转派、接受/拒绝的身份只取自登录会话；演示账号 `wangmin`、`lilei`、`zhaoqian`，密码均为 `rentfind123`。
